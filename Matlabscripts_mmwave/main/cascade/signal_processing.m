...
%     "D:\RadarDataset\Flying_Drone_Data\3GHz_parameters\phantom4\",
%     "D:\RadarDataset\Flying_Drone_Data\3GHz_parameters\mavic3\",
%     "D:\RadarDataset\Flying_Drone_Data\3GHz_parameters\mavic2\",
%     "D:\RadarDataset\Flying_Drone_Data\3GHz_parameters\environment2\",
%    
%     
% ];
folder_paths_of_data = [
      ...
      "D:\RadarDataset\20241210_phantom_ang3\",
%       "D:\RadarDataset\20241210_Rcplane_ang3\",
       %"D:\RadarDataset\Flying_Drone_Data\3GHz_parameters\mavic3\",
%      "D:\RadarDataset\Flying_Drone_Data\20250106_phantom4fly_ang1\", 
%      "D:\RadarDataset\20241210_environment2\",
];
...
for frameIdx = 2:2;
       ...
       [numValidFrames dataFileSize] = getValidNumFrames(fullfile(dataFolder_test, fileNameStruct.masterIdxFile));
        ...
        switch i
            ...
            case 1
                ...
                radar_data_1= adcData;
                %radar_data_1= processAndFilter(radar_data_1, b, a);
                random_value = 2 * rand();
                radar_data_1=applyRangeShift(radar_data_1,0);
                fprintf("Store one file radar_data1: %d\n", i);
            case 2
                radar_data_2 = adcData;
                radar_data_2= processAndFilter(radar_data_2, b, a);
                random_value = 2 * rand();

                radar_data_2=applyRangeShift(radar_data_2,range_value);
    ...
    i=1;
    if i==1
        %adcData = radar_data_1+radar_data_2+radar_data_3-radar_data_4-radar_data_4-radar_data_4 ; 
        %adcData=radar_data_1+radar_data_2-radar_data_3-radar_data_3;
        %adcData=radar_data_2+radar_data_1-radar_data_3-radar_data_3;
        adcData=radar_data_1;
        % Specify the output directory for the wavelet plots
        outputDir = 'D:\DroneSwarmsImages\RangeShifteData\SinglePhnotomFilteraApplied';
...
function shiftedData = applyRangeShift(adcData, delta_r)
...
function plotWaveletForChirps(adcData, outputDir, frame_index_val)
    ...
    for groupIdx = 1:numGroups
        ...
        startChirp = (groupIdx - 1) * chirpsPerGroup + 1;
        endChirp = min(groupIdx * chirpsPerGroup, totalChirps);
        disp("The adcData size befor applying wavelet");
        disp(size(adcData));
        averageData = mean(adcData, 3);
        % Reshape the averaged data to 256x128x1
        reshapedData = reshape(averageData, [256, 128, 1]);
        disp(size( averageData ));
        % Select data for the current chirp group
        selectedData =  reshapedData(:, startChirp:endChirp, 1); % Assuming antenna index 1
       ...
       % Compute magnitude of wavelet coefficients
        ...
        figure;
        h_mag = pcolor(t, f_real, abs(cfs_real).^2);
        ...
        xlabel('Time (s)');
        ylabel('Frequency (Hz)');
        title('Magnitude of Wavelet Coefficients');
        ...
        hold on;
        contour(t, f_real, abs(cfs_real).^2, 'LineWidth', 1, 'LineColor', 'k'); % Black contour lines
        hold off;

        % Save the plot with axis and labels
        saveas(gcf, fullfile(outputDir, sprintf('imag%d_wavelet%d_real.png', frame_index_val, groupIdx)));
...
function filteredMatrix = processAndFilter(inputMatrix, b, a)
    % Reshape to 3D for processing
    reshapedMatrix = reshape(inputMatrix, size(inputMatrix, 1), size(inputMatrix, 2), size(inputMatrix, 3) * size(inputMatrix, 4));
    disp(['The dimensions of the reshaped matrix are: ', num2str(size(reshapedMatrix))]);
    
    % Apply the high-pass filter on real and imaginary parts
    for antennaIdx = 1:size(reshapedMatrix, 3)
        realPart = real(reshapedMatrix(:, :, antennaIdx));
        imagPart = imag(reshapedMatrix(:, :, antennaIdx));
        ...
        reshapedMatrix(:, :, antennaIdx) = complex(filteredReal, filteredImag); % Combine filtered components
    end
    
    % Reshape back to the original dimensions
    filteredMatrix = reshape(reshapedMatrix,256,128,16,12);
end
